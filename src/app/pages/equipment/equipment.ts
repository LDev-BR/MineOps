...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2 lg:pb-5 shrink-0">
      <div>
        <h1 class="text-md lg:text-2xl font-bold font-mono tracking-wide text-slate-100 flex items-center gap-1.5 lg:gap-2">
          <span class="w-2.5 h-2.5 rounded bg-amber-500 lg:hidden"></span>
          <mat-icon class="text-amber-500 hidden lg:inline">construction</mat-icon> {{ isPt() ? 'ATIVOS DA FROTA PESADA' : 'HEAVY FLEET ASSETS' }}
        </h1>
        <p class="text-[9px] lg:text-xs text-slate-400 mt-0.5 uppercase font-mono tracking-wider">
      ...
      <!-- Action Button: Register Asset (hidden on mobile, shown on lg+) -->
      <button (click)="openRegistrationModal()" class="hidden lg:flex bg-amber-500 hover:bg-amber-600 text-slate-950 font-mono font-bold text-xs px-4 py-2 rounded shadow transition items-center gap-1.5 self-start md:self-auto cursor-pointer">
    ...
    <div class="flex flex-col gap-1.5 shrink-0 lg:hidden">
      <div class="flex items-center justify-between bg-slate-950 p-1.5 rounded-lg border border-slate-850 select-none">
        <button (click)="prevMobilePanel()" class="p-1 px-2.5 bg-slate-900 border border-slate-800 hover:bg-slate-850 rounded text-slate-400 hover:text-slate-100 transition flex items-center justify-center cursor-pointer focus:outline-none" aria-label="Previous Panel">
          <mat-icon class="text-sm">chevron_left</mat-icon>
        </button>
        <div class="flex flex-col items-center">
          <span class="text-[8px] font-mono font-bold text-amber-500 uppercase tracking-widest leading-none">{{ isPt() ? 'PAINEL' : 'PANEL' }} {{ activeMobilePanel() + 1 }} {{ isPt() ? 'DE' : 'OF' }} {{ mobilePanelCount() }}</span>
          <span class="text-[10px] font-bold text-slate-250 uppercase mt-1 tracking-wide leading-none">{{ activeMobilePanelName() }}</span>
        </div>
        <button (click)="nextMobilePanel()" class="p-1 px-2.5 bg-slate-900 border border-slate-800 hover:bg-slate-850 rounded text-slate-400 hover:text-slate-100 transition flex items-center justify-center cursor-pointer focus:outline-none" aria-label="Next Panel">
      ...
      <div class="flex justify-center gap-1.5 select-none pb-0.5">
        @for (p of mobilePanels; track p.id; let idx = $index) {
          <button (click)="activeMobilePanel.set(idx)"
               class="h-1.5 rounded-full transition-all duration-300 cursor-pointer border-0 p-0 focus:outline-none"
               [class.w-5]="activeMobilePanel() === idx"
               [class.bg-amber-500]="activeMobilePanel() === idx"
               [class.w-1.5]="activeMobilePanel() !== idx"
               [class.bg-slate-800]="activeMobilePanel() !== idx"
               [class.hover:bg-slate-600]="activeMobilePanel() !== idx"
               [attr.aria-label]="'Go to panel ' + (idx + 1)"></button>
        }
      </div>
    </div>

    <!-- Quick ERP Actions (Mobile only) -->
    <div class="flex gap-2 justify-end items-center shrink-0 select-none lg:hidden">
      <button (click)="openRegistrationModal()"
              class="px-2.5 py-1 bg-[#F97316] hover:bg-[#EA580C] text-white rounded text-[10px] font-bold font-sans tracking-wide transition flex items-center gap-1.5 shadow-md cursor-pointer">
        <mat-icon class="text-xs h-3.5 w-3.5 flex items-center justify-center">add</mat-icon>
        {{ isPt() ? 'REGISTRAR' : 'REGISTER' }}
      </button>
    </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-h-0 relative">
      ...
      <!-- ==================== DESKTOP LAYOUT (lg+) ==================== -->
      <div class="hidden lg:grid grid-cols-1 xl:grid-cols-12 gap-6 items-start relative flex-1 overflow-y-auto">
        ...
        <!-- Detail Slide-Over / Static Panel Column (Spans 4) - DESKTOP ONLY -->
      ...
      <div class="flex lg:hidden flex-col flex-1 overflow-hidden min-h-0">
        ...
        <!-- MOBILE PANEL 1: Asset List (Card View) -->
        @if (activeMobilePanel() === 0) {
          <div class="flex flex-col flex-1 overflow-hidden animate-fade-in gap-1.5">
            
            <!-- Compact filters -->
            <div class="flex gap-1.5 items-center shrink-0">
              <div class="relative flex-1">
                <input [value]="searchQuery()" (input)="onSearchChange($event)" type="text" [placeholder]="isPt() ? 'Buscar ativo...' : 'Search asset...'"
                       class="w-full bg-slate-950 border border-slate-700 rounded pl-7 pr-2 py-1.5 text-[10px] text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500 font-mono">
                <mat-icon class="absolute left-2 top-1.5 text-slate-550 text-xs">search</mat-icon>
              </div>
              <select [value]="statusFilter()" (change)="onStatusFilterChange($event)"
                      class="bg-slate-950 border border-slate-700 text-slate-300 rounded px-2 py-1.5 text-[10px] font-mono focus:outline-none focus:border-amber-500 cursor-pointer max-w-[120px]">
                <option value="ALL">{{ isPt() ? 'Todos' : 'All' }}</option>
                <option value="Operational">{{ isPt() ? 'Oper.' : 'Oper.' }}</option>
                <option value="Under Maintenance">{{ isPt() ? 'Manut.' : 'Maint.' }}</option>
                <option value="Standby">Standby</option>
                <option value="Breakdown">{{ isPt() ? 'Falha' : 'Break.' }}</option>
              </select>
              @if (hasActiveFilters()) {
                <button (click)="resetFilters()" class="text-amber-500 p-1 cursor-pointer">
            ...
            <div class="flex-1 overflow-y-auto min-h-0 flex flex-col gap-1.5 pr-0.5 scrollbar-thin">
              @if (paginatedEquipments().length === 0) {
                <div class="flex-1 flex items-center justify-center text-slate-500 font-mono text-[10px]">
                  {{ isPt() ? 'Nenhum ativo encontrado.' : 'No assets found.' }}
                </div>
              } @else {
                @for (eq of paginatedEquipments(); track eq.id) {
                  <div (click)="selectEquipmentMobile(eq)" (keydown.enter)="selectEquipmentMobile(eq)" tabindex="0" role="button"
                       class="bg-slate-850 border border-slate-800 rounded-lg p-2.5 flex flex-col gap-1.5 cursor-pointer hover:border-slate-700 transition select-none"
                       [class.border-amber-500]="selectedEquipment()?.id === eq.id"
                       [class.bg-slate-800]="selectedEquipment()?.id === eq.id">
                    
                    <!-- Top row: ID + Status badge -->
                    <div class="flex justify-between items-center">
                      <div class="flex items-center gap-2">
                        <div class="w-6 h-6 rounded-sm flex items-center justify-center font-mono font-bold text-[9px] shadow-sm"
                             [ngClass]="{
                               'bg-emerald-950 text-emerald-400 border border-emerald-800': eq.healthScore >= 90,
                               'bg-amber-950/80 text-amber-500 border border-amber-800': eq.healthScore < 90 && eq.healthScore >= 70,
                               'bg-rose-950 text-rose-500 border border-rose-800': eq.healthScore < 70
                        ...
                        <span class="font-mono font-bold text-[10px] text-slate-300">{{ eq.id }}</span>
                      </div>
                      <span class="text-[8px] font-mono px-1.5 py-0.5 rounded font-bold uppercase text-slate-100"
                            [ngClass]="{
                              'bg-emerald-600': eq.status === 'Operational',
                              'bg-amber-505 text-slate-950': eq.status === 'Under Maintenance',
                              'bg-indigo-600': eq.status === 'Standby',
                    ...
                    <div>
                      <span class="font-bold text-[11px] text-slate-200 block leading-tight">{{ eq.name }}</span>
                      <span class="text-[9px] text-slate-450 font-mono">{{ eq.model }} &bull; {{ eq.manufacturer }}</span>
                    </div>

                    <!-- Bottom info row -->
                    <div class="flex justify-between items-center text-[8px] font-mono text-slate-500 border-t border-slate-800/60 pt-1">
                      <span class="flex items-center gap-0.5"><mat-icon class="text-[10px] h-2.5 w-2.5">place</mat-icon> {{ eq.location }}</span>
                      <span>{{ eq.operationalHours | number }} h</span>
                    </div>
                  </div>
                }
              }
            ...
            <!-- Compact pagination -->
            <div class="bg-slate-950 border border-slate-800 rounded-lg p-1.5 flex items-center justify-between text-[9px] font-mono text-slate-450 select-none shrink-0">
              <span>{{ startRecord() }}-{{ endRecord() }} / {{ filteredEquipments().length }}</span>
              <div class="flex items-center gap-1.5">
                <button (click)="prevPage()" [disabled]="currentPage() === 1" class="p-0.5 px-1.5 bg-slate-900 rounded border border-slate-800 text-slate-400 disabled:opacity-40 transition cursor-pointer">
                  <mat-icon class="text-xs h-3 w-3">chevron_left</mat-icon>
                </button>
                <span class="text-slate-350">{{ currentPage() }}/{{ totalPages() || 1 }}</span>
                <button (click)="nextPage()" [disabled]="currentPage() === totalPages() || totalPages() === 0" class="p-0.5 px-1.5 bg-slate-900 rounded border border-slate-800 text-slate-400 disabled:opacity-40 transition cursor-pointer">
        ...
        <!-- MOBILE PANEL 2: Asset Details -->
        @if (activeMobilePanel() === 1) {
          <div class="flex flex-col flex-1 overflow-hidden animate-fade-in gap-1.5">
            @if (selectedEquipment(); as eq) {
              <div class="bg-slate-850 border border-slate-800 rounded-lg flex flex-col flex-1 overflow-y-auto scrollbar-thin">
                ...
                <div class="p-3 flex flex-col gap-3 text-xs">
                  ...
                  <div class="flex justify-between items-start border-b border-slate-800 pb-2">
                    <div>
                      <p class="text-[8px] font-mono text-slate-500 font-bold tracking-wider uppercase">{{ isPt() ? 'DOSSIÊ DO ATIVO' : 'ASSET DOSSIER' }}</p>
                      <h2 class="text-sm font-bold font-mono text-slate-100 mt-0.5">{{ eq.id }}</h2>
                      <p class="text-[10px] text-slate-350">{{ eq.name }}</p>
                    </div>
                    <span class="text-[8px] font-mono px-1.5 py-0.5 rounded font-bold uppercase text-slate-100"
                          [ngClass]="{
                            'bg-emerald-600': eq.status === 'Operational',
                  ...
                  <!-- State Controls -->
                  <div class="p-2 bg-slate-900 border border-slate-800 rounded flex flex-col gap-1.5">
                    <span class="text-[8px] font-bold font-mono text-slate-500 uppercase tracking-widest">{{ isPt() ? 'FORÇAR ESTADO' : 'OVERRIDE STATE' }}</span>
                    <div class="flex gap-1">
                      <button (click)="changeAssetStatus(eq.id, 'Operational')" 
                              class="flex-1 py-1 bg-slate-800 border border-slate-700 rounded text-[8px] font-mono font-bold uppercase text-emerald-400 cursor-pointer"
                              [class.bg-emerald-950]="eq.status === 'Operational'">
                        {{ isPt() ? 'Oper.' : 'Oper.' }}
                      </button>
                      <button (click)="changeAssetStatus(eq.id, 'Under Maintenance')" 
                              class="flex-1 py-1 bg-slate-800 border border-slate-700 rounded text-[8px] font-mono font-bold uppercase text-amber-400 cursor-pointer"
                              [class.bg-amber-950]="eq.status === 'Under Maintenance'">
                        {{ isPt() ? 'Manut.' : 'Maint.' }}
                      </button>
                      <button (click)="changeAssetStatus(eq.id, 'Breakdown')" 
                              class="flex-1 py-1 bg-slate-800 border border-slate-700 rounded text-[8px] font-mono font-bold uppercase text-rose-500 cursor-pointer"
                              [class.bg-rose-950]="eq.status === 'Breakdown'">
                        {{ isPt() ? 'Falha' : 'Break.' }}
                      </button>
                    </div>
                  ...
                  <div class="grid grid-cols-2 gap-2">
                    <div class="bg-slate-900/60 p-2 rounded border border-slate-750 font-mono">
                      <span class="text-slate-500 block text-[8px] uppercase font-bold">{{ isPt() ? 'TIPO' : 'TYPE' }}</span>
                      <span class="text-slate-200 block text-[10px] font-bold mt-0.5">{{ eq.type }}</span>
                    </div>
                    <div class="bg-slate-900/60 p-2 rounded border border-slate-750 font-mono">
                      <span class="text-slate-500 block text-[8px] uppercase font-bold">{{ isPt() ? 'FABRICANTE' : 'MANUF.' }}</span>
                      <span class="text-slate-200 block text-[10px] font-bold mt-0.5">{{ eq.manufacturer }}</span>
                    </div>
                    <div class="bg-slate-900/60 p-2 rounded border border-slate-750 font-mono">
                      <span class="text-slate-500 block text-[8px] uppercase font-bold">{{ isPt() ? 'MODELO' : 'MODEL' }}</span>
                      <span class="text-slate-200 block text-[10px] font-bold mt-0.5">{{ eq.model }}</span>
                    </div>
                    <div class="bg-slate-900/60 p-2 rounded border border-slate-750 font-mono">
                      <span class="text-slate-500 block text-[8px] uppercase font-bold">{{ isPt() ? 'SAÚDE' : 'HEALTH' }}</span>
                      <span class="font-bold block text-[10px] mt-0.5"
                            [ngClass]="{'text-emerald-455': eq.healthScore >= 90, 'text-amber-500': eq.healthScore < 90 && eq.healthScore >= 70, 'text-rose-500': eq.healthScore < 70}">
                        {{ eq.healthScore }}%
                      </span>
                    </div>
                  ...
                  <!-- Fuel + Telemetry -->
                  <div class="bg-slate-950 p-2 rounded border border-slate-800 flex flex-col gap-2 font-mono">
                    <div class="flex flex-col gap-0.5">
                      <div class="flex justify-between text-[9px] text-slate-450">
                        <span>{{ isPt() ? 'COMBUSTÍVEL' : 'FUEL LEVEL' }}</span>
                        <span class="font-bold" [class.text-amber-400]="eq.fuelLevel < 35">{{ eq.fuelLevel }}%</span>
                      </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 text-[9px] text-slate-400">
                      <div class="border-l-2 border-slate-700 pl-1.5">
                        <span class="text-slate-550 block text-[7px] uppercase">{{ isPt() ? 'Temp. Motor' : 'Engine Temp' }}</span>
                        <span class="text-slate-200 font-bold text-[10px]" [class.text-rose-400]="tempSim(eq.status) > 105">{{ tempSim(eq.status) }}&deg;C</span>
                      </div>
                      <div class="border-l-2 border-slate-700 pl-1.5">
                        <span class="text-slate-550 block text-[7px] uppercase">{{ isPt() ? 'Vibrações' : 'Vibrations' }}</span>
                        <span class="text-slate-200 font-bold text-[10px]" [class.text-rose-400]="vibSim(eq.status) > 12">{{ vibSim(eq.status) }} mm/s</span>
                      </div>
                    </div>
                  </div>

                  <!-- Maintenance dates -->
                  <div class="p-2 bg-slate-900/40 border border-slate-750 rounded font-mono text-[10px] text-slate-400">
                    <div class="flex justify-between pb-1 mb-1 border-b border-slate-750/60">
                      <span>{{ isPt() ? 'ÚLTIMA PM:' : 'LAST PM:' }}</span>
                      <span class="text-slate-200 font-bold">{{ eq.lastMaintenanceDate }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span>{{ isPt() ? 'PRÓXIMA PM:' : 'NEXT PM:' }}</span>
                      <span class="text-amber-500 font-bold">{{ eq.nextMaintenanceDate }}</span>
                    </div>
                  </div>

                  <!-- Coordinates -->
                  <div class="p-2 bg-slate-950 font-mono text-[9px] text-slate-450 border border-slate-800 rounded flex flex-col gap-0.5">
                    <div class="flex justify-between items-center mb-0.5">
                      <span class="text-[8px] text-slate-500 font-bold uppercase">{{ isPt() ? 'COORDENADAS' : 'COORDINATES' }}</span>
                      <span class="text-[8px] text-emerald-500 font-black">● {{ isPt() ? 'LOCALIZADO' : 'LOCATED' }}</span>
                    </div>
                    <div>LAT: <span class="text-slate-200 font-bold">{{ eq.latitude | number:'1.6-6' }}</span></div>
                    <div>LON: <span class="text-slate-200 font-bold">{{ eq.longitude | number:'1.6-6' }}</span></div>
                  </div>

                  <!-- Crew -->
                  @if (getTeamForEquipment(eq.assignedTeamId); as crew) {
                    <div class="p-2 bg-slate-900 border border-slate-750 rounded flex gap-2 items-center">
                      <div class="w-7 h-7 rounded bg-slate-850 border border-slate-700 flex items-center justify-center text-slate-300">
                        <mat-icon class="text-sm">groups</mat-icon>
                      </div>
                      <div>
                        <p class="font-bold text-slate-250 text-[10px]">{{ crew.name }}</p>
                        <p class="text-[8px] text-slate-450 font-mono uppercase">{{ isPt() ? 'Supervisor:' : 'Supervisor:' }} <span class="text-amber-500 font-bold">{{ crew.supervisor }}</span></p>
                      </div>
                    </div>
                  }
                </div>
              ...
              <div class="flex-1 flex flex-col items-center justify-center gap-2 text-slate-500 select-none">
                <mat-icon class="text-3xl text-slate-700">touch_app</mat-icon>
                <p class="text-[10px] font-mono text-center">{{ isPt() ? 'Selecione um ativo na lista para ver detalhes.' : 'Select an asset from the list to view details.' }}</p>
                <button (click)="activeMobilePanel.set(0)" class="text-[9px] font-mono text-amber-500 font-bold uppercase cursor-pointer mt-1">
                  {{ isPt() ? '← IR PARA LISTA' : '← GO TO LIST' }}
                </button>
              </div>
            }
          </div>
    ...
    @if (modalOpen()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div class="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
          <div class="h-1 bg-amber-500 shrink-0"></div>
          
          <div class="p-3 lg:p-5 bg-slate-950 border-b border-slate-800 flex justify-between items-center text-slate-100 shrink-0">
            <h3 class="font-mono font-bold text-xs lg:text-sm tracking-wide uppercase flex items-center gap-2">
              <mat-icon class="text-amber-500">add_to_photos</mat-icon> {{ isPt() ? 'Registrar Máquina' : 'Machinery Entry Declaration' }}
            </h3>
          ...
          <div class="p-3 lg:p-5 flex flex-col gap-3 lg:gap-4 text-xs font-sans text-slate-300 overflow-y-auto">
            <div class="grid grid-cols-2 gap-3 lg:gap-4">
              <div class="flex flex-col gap-1">
                <label for="eq-id-input" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Cód. do Ativo' : 'Unique Asset ID' }}</label>
                <input id="eq-id-input" #newId type="text" placeholder="EQ-EXC-215" 
                       class="bg-slate-950 border border-slate-700 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs font-mono text-slate-200 placeholder-slate-650 focus:outline-none focus:border-amber-500">
              </div>
              <div class="flex flex-col gap-1">
                <label for="eq-name-input" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Nome da Máquina' : 'Machine Name' }}</label>
                <input id="eq-name-input" #newName type="text" placeholder="Caterpillar High Cap"
                       class="bg-slate-950 border border-slate-700 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500">
              </div>
              <div class="flex flex-col gap-1">
                <label for="eq-model-input" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Modelo' : 'Model' }}</label>
                <input id="eq-model-input" #newModel type="text" placeholder="6060 FS"
                       class="bg-slate-950 border border-slate-700 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs font-mono text-slate-200 placeholder-slate-650 focus:outline-none focus:border-amber-500">
              </div>
              <div class="flex flex-col gap-1">
                <label for="eq-type-select" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Tipo' : 'Fleet Type' }}</label>
                <select id="eq-type-select" #newType class="bg-slate-950 border border-slate-700 text-slate-300 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs font-mono focus:outline-none focus:border-amber-500">
                  <option value="Excavator">{{ isPt() ? 'Escavadeira' : 'Excavator' }}</option>
                  <option value="Dump Truck">{{ isPt() ? 'Caminhão' : 'Dump Truck' }}</option>
                  <option value="Drilling Machine">{{ isPt() ? 'Perfuratriz' : 'Rotary Drill' }}</option>
                  <option value="Loader">{{ isPt() ? 'Carregadeira' : 'Loader' }}</option>
                  <option value="Bulldozer">{{ isPt() ? 'Trator' : 'Bulldozer' }}</option>
                </select>
              </div>
              <div class="flex flex-col gap-1">
                <label for="eq-manuf-input" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Fabricante' : 'Manufacturer' }}</label>
                <input id="eq-manuf-input" #newManuf type="text" placeholder="Caterpillar"
                       class="bg-slate-950 border border-slate-700 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500">
              </div>
              <div class="flex flex-col gap-1">
                <label for="eq-team-select" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Equipe' : 'Team' }}</label>
                <select id="eq-team-select" #newTeam class="bg-slate-950 border border-slate-700 text-slate-300 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs font-mono focus:outline-none focus:border-amber-500">
                  <option value="TM-01">Alpha Heavy Shovel Crew</option>
                  <option value="TM-02">Alpha Haulage Crew</option>
                  <option value="TM-03">Delta Maintenance</option>
                  <option value="TM-04">Beta Drills</option>
                  <option value="TM-05">Crusherfeed Crew</option>
                </select>
              </div>
            </div>
            
            <div class="flex flex-col gap-1">
              <label for="eq-loc-input" class="text-[9px] lg:text-[10px] font-bold font-mono uppercase tracking-wider text-slate-400">{{ isPt() ? 'Localização' : 'Location' }}</label>
              <input id="eq-loc-input" #newLoc type="text" placeholder="Pit Alpha - East Bench"
                     class="bg-slate-950 border border-slate-700 rounded px-2 lg:px-3 py-1.5 lg:py-2 text-[10px] lg:text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-amber-500">
            </div>

            <div class="flex gap-2 mt-2 lg:mt-4 justify-end">
              <button (click)="closeRegistrationModal()" class="py-1.5 lg:py-2 px-3 lg:px-4 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200 text-[10px] lg:text-xs font-mono rounded transition cursor-pointer">
              ...
              <button (click)="registerMachinery(newId.value, newName.value, newModel.value, newType.value, newManuf.value, newLoc.value, newTeam.value)" 
                      class="py-1.5 lg:py-2 px-4 lg:px-5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-[10px] lg:text-xs font-mono font-bold rounded transition tracking-wider cursor-pointer">
                {{ isPt() ? 'REGISTRAR' : 'COMMIT TO FLEET' }}
              </button>
            </div>
          </div>
        </div>
...
export class EquipmentManagement {
  ...
  public activeMobilePanel = signal<number>(0);
  
  public mobilePanels = [
    { id: 'list', nameEn: 'ASSET LIST', namePt: 'LISTA DE ATIVOS' },
    { id: 'details', nameEn: 'ASSET DETAILS', namePt: 'DETALHES DO ATIVO' }
  ];
  
  public mobilePanelCount = computed(() => this.mobilePanels.length);
  
  public activeMobilePanelName = computed(() => {
    const panel = this.mobilePanels[this.activeMobilePanel()];
  ...
  // Mobile panel navigation
  public prevMobilePanel() {
    this.activeMobilePanel.update(p => p > 0 ? p - 1 : this.mobilePanels.length - 1);
  }
  
  public nextMobilePanel() {
    this.activeMobilePanel.update(p => p < this.mobilePanels.length - 1 ? p + 1 : 0);
  }

  // Select equipment and navigate to details on mobile
  public selectEquipmentMobile(eq: Equipment) {
    this.selectedEquipment.set(eq);
    this.activeMobilePanel.set(1); // Navigate to details panel
  }
